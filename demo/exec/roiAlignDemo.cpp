...
int main(int argc, char* argv[]) {
    if (argc < 2) {
        std::cout << "Usage: ./roiAlignDemo.out model.mnn" << std::endl;
    }
    ...
    const std::vector<float> inputData  = {// [0, 0, :, :]
                                          -0.2280, 1.0844, -0.5641, 1.0726, -1.3492, 1.7201, 0.9563, -0.0467, 0.6715,
                                          1.9948, -0.8688, 0.7443, 1.2673, -0.8053, -0.5623, -1.0905,
                                          // [0, 1, :, :]
                                          1.3876, 0.0851, -0.2216, 0.9205, -0.2518, 0.3693, -0.8745, -0.8473, -1.2684,
                                          -0.5659, 1.7629, -1.1017, -0.6808, -0.3398, 0.3905, -1.4468,
                                          // [0, 2, :, :]
                                          0.3544, -1.3111, -0.4820, -0.7897, -0.6373, 0.2016, 0.6290, -0.4031, -1.1968,
                                          1.2157, 0.7829, 0.0650, 1.2640, -0.1800, 0.5405, 0.5089,
                                          // [0, 3, :, :]
                                          -1.0524, -0.3499, -1.0782, 2.3311, 0.3991, -1.0441, 1.4817, -0.1174, -1.2182,
                                          1.6637, 0.0859, 0.2630, 0.1632, -0.7833, 1.9403, 2.7318,
                                          // [0, 4, :, :]
                                          1.9020, -0.6179, -1.9794, -0.5269, 1.7679, -1.0336, 0.6070, 0.9047, 0.5333,
                                          0.0791, -1.3838, -2.0784, 0.9168, -0.2555, 1.6408, 0.7012,
                                          // [1, 0, :, :]
                                          -1.3412, 0.1940, 1.1810, -0.1982, -0.6593, 1.3539, -1.0911, 0.7103, 0.0096,
                                          0.4430, -0.0726, 0.3056, -0.5561, -0.0727, 1.3280, -0.1100,
                                          // [1, 1, :, :]
                                          2.1148, -1.5199, -1.7310, -1.2742, -0.5361, -0.1515, 0.3356, -0.4218, -0.0041,
                                          -0.1434, 1.1420, 0.5182, -0.0388, 1.4477, -0.0358, -1.3596,
                                          // [1, 2, :, :]
                                          -0.5037, -1.2696, 0.1072, 0.4771, 1.0347, -0.8084, 0.0574, -0.2152, -0.4784,
                                          -0.1899, -0.4062, -1.2771, -0.1168, 0.9634, 0.8064, 1.3298,
                                          // [1, 3, :, :]
                                          0.9451, 0.5655, 1.3067, -2.3284, -1.0857, 0.0211, 0.2268, -0.4762, 0.4864,
                                          -0.0473, 0.1158, 0.1223, -0.2063, -0.6610, -1.3957, 0.5784,
                                          // [1, 4, :, :]
                                          -0.6520, 1.7823, 1.6848, -1.3009, 0.8962, 0.7669, -0.1907, 0.0051, -0.2227,
                                          0.0253, -0.3897, -1.2509, 1.0388, 1.2419, 1.3504, 0.9589,
                                          // [2, 0, :, :]
                                          -0.6821, -1.3088, 0.6674, 0.3037, -0.6739, 0.7570, 1.2381, 0.9869, 0.6168,
                                          0.3319, 0.0818, -1.4019, 0.4963, -0.6783, -0.3395, 0.1034,
                                          // [2, 1, :, :]
                                          -1.6103, 0.1561, -1.8178, -0.6959, 0.2309, -2.1099, 0.2700, 1.1527, -0.1562,
                                          -0.8010, -1.7923, 1.9186, 0.0420, 1.0442, 0.4630, -1.7146,
                                          // [2, 2, :, :]
                                          ...
                                          // [2, 4, :, :]
                                          0.4548, -0.4797, 0.3163, -0.3530, 1.0514, -0.9240, 1.1464, 0.1866, -0.1598,
                                          0.1525, 0.9954, 2.0155, -0.0096, -0.3440, 1.0122, 0.5473};
    ...
    MNN::Tensor* roisHostTensor = MNN::Tensor::create<float>(rois->shape(), (void*)roiData.data(), MNN::Tensor::CAFFE);
